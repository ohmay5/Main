		...
		if tostring(I) == "FireServer" then
			if tostring(e[1]) == "RemoteEvent" then
				if tostring(e[2]) ~= "true" and tostring(e[2]) ~= "false" then
					if _G.FarmMastery_G and not SoulGuitar or _G.FarmMastery_Dev or _G.FarmBlazeEM or _G.Prehis_Skills or _G.SeaBeast1 or _G.FishBoat or _G.PGB or _G.Leviathan1 or _G.Complete_Trials then
						e[2] = MousePos;
						return i(unpack(e));
					end;
				end;
...
Hop = function()
    pcall(function()
        local ServerBrowser =
            replicated:WaitForChild("ServerBrowser")

        for Page = math.random(1, math.random(40, 75)), 100 do
            local Servers =
                ServerBrowser:InvokeServer(Page)

            if type(Servers) == "table" then
                for JobId, Info in pairs(Servers) do
                    if tonumber(Info.Count)
                        and tonumber(Info.Count) < 12 then
...
QuestNeta = function()
	...
	local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/Omgshit/BloxFruits/refs/heads/main/UI"))():MakeWindow({
    Title = "BaCoNhǎo | Hub",
    SubTitle = "Blox Fruit",
...
Status:AddButton({
    ...
    Callback = function()
        ...
        local serversUrl =
            Api .. placeId .. "/servers/Public?sortOrder=Asc&limit=100"
        ...
        repeat
            ...
            for _, Server in ipairs(Servers.data or {}) do
                if Server.id ~= game.JobId
                    and Server.playing
                    and Server.playing < LowestPlayers then
                    ...
                    if plr.Character.Humanoid.Sit == false then
                        ...
                        repeat
                            ...
                            if not _G.FishBoat and CheckEnemiesBoat()
                                ...
                                -- Bình thường: BAY +60
                                _tp(CFrameSelectedZone * CFrame.new(0, 80, 0))
