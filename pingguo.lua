...
-- Bản hoàn chỉnh - Script tự động làm nhiệm vụ Saber
spawn(function()
    while task.wait(1) do
        ...
        pcall(function()
            ...
            -- Kiểm tra nếu đã có kiếm thì dừng
            if plr.Backpack:FindFirstChild("Saber") or char:FindFirstChild("Saber") then
                _G.AutoSaber = false
                return
            end

            -- LẤY TRẠNG THÁI TỪ SERVER (Chìa khóa để không bị loop)
            ...
            -- CẤU TRÚC TUẦN TỰ (Từ bước cuối lên bước đầu)
            
            -- Bước 5: Đánh Saber Expert
            if workspace.Enemies:FindFirstChild("Saber Expert") then
                local boss = workspace.Enemies["Saber Expert"]
            ...
            -- Bước 4: Đã xong RichSon, đến vị trí Saber Expert
            elseif richSonStatus == 1 then
                EquipWeapon("Relic")
                _tp(CFrame.new(-1404.91, 29.97, 3.80))
            
            -- Bước 3: Đánh Mob Leader (RichSon)
            ...
            -- Bước 2: Làm nhiệm vụ Cốc (SickMan)
            ...
            -- Bước 1.2: Đốt cửa Desert (Chỉ làm nếu chưa xong SickMan)
            ...
            -- Bước 1.1: Nhấn nút Jungle (Bước cuối cùng mới kiểm tra)
            elseif workspace.Map.Jungle.Final.Part.Transparency == 0 then
                if workspace.Map.Jungle.QuestPlates.Door.Transparency == 0 then
                    ...
                    for _, p in ipairs(plates) do
                        local btn = workspace.Map.Jungle.QuestPlates:FindFirstChild(p)
                        if btn then root.CFrame = btn.Button.CFrame; task.wait(0.3) end
                    end
                else
