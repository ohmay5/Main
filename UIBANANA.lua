if getgenv().Nousigi then 
	if game.CoreGui:FindFirstChild("BaCoN Hub GUI") then
		for i, v in ipairs(game.CoreGui:GetChildren()) do
			if string.find(v.Name,  "BaCoN Hub") then
				v:Destroy()
			end
		end
	end
...
function Library:CreateWindow(Setting)
    local TitleNameMain = Setting.Name or "BaConHǎo Hub"
	...
	maingui.Image = "rbxassetid://114476175638281"
	...
	UIPage.TweenTime = 0.5 getgenv().UIColor["Tween Animation 1 Speed"]
	...
	function Main_Function:AddTab(PageName)
        ...
        function sectionFunction:AddButton(Setting)
        	local Title = Setting.Name or Setting.Text or ""
    ...
    RunService.RenderStepped:Connect(function()
        if dragging and dragInput then
            local percentage = math.clamp(
                (dragInput.Position.X - SliderBar.AbsolutePosition.X)
...
function sectionFunction:AddKeyBind(Setting)
    local TitleText = tostring(Setting.Name or Setting.Text) or ""
    ...
    return keybindFunction
			...
			function sectionFunction:AddTextbox(Setting)
	local TitleText = tostring(Setting.Name or Setting.Text) or ""
	...
	Boxxx.Focused:Connect(function()
		TweenService:Create(Lineeeee, TweenInfo.new(
			getgenv().UIColor["Tween Animation 2 Speed"]
		), {
			BackgroundTransparency = 0
		}):Play()
	end)
