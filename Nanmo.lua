...
if I >= 1500 and I <= 1524 then
    ...
    PosQ = CFrame.new(-448.05, 108.60, 5948.64)
    PosM1 = CFrame.new(-636.94, 57.01, 5621.92)
    PosM2 = CFrame.new(-133.49, 57.01, 5759.59)
...
elseif I >= 1525 and I <= 1574 then
    ...
    PosQ = CFrame.new(-448.05, 108.60, 5948.64) -- cùng NPC nhận nhiệm vụ
...
local Farm = Window:AddTab("Tab Farming")
local Others = Window:AddTab("Stack Farm")
local Maestry = Window:AddTab("Farm Maestry")
local Event = Window:AddTab("Sea Event")
local Race = Window:AddTab("Upgrade V4")
local Dojo = Window:AddTab("Dojo & Drago Race")
local Get = Window:AddTab("Get Item & Upgrade")
local Fruit = Window:AddTab("Raid & Fruit")
local Player = Window:AddTab("Local Player")
local Shop = Window:AddTab("Local Shop")
    ...
    while task.wait(Sec) do
        pcall(function()
            ...
            elseif Progress == 1 then
                ...
                -- kiểm tra hoàn thành sau khi giết đủ 3 con
                local Check = replicated.Remotes.CommF_:InvokeServer("Wenlocktoad", "1")
                if Check == 2 then
...
Race:AddToggle({
    Name = "Auto Race V3",
    Description = "Auto Human/Mink/Angel/Shark V3",
    Default = false,
    Callback = function(v)
        G.AutoRaceV3 = v
    end,
...
spawn(function()
    while task.wait(Sec) do
        pcall(function()
            if not G.AutoRaceV3 then return end
            ...
            elseif Progress == 1 then
                ...
                if Race == "Human" then
                    ...
                    if Mob then
                        repeat
                            task.wait()
                            G.Kill(Mob, G.AutoRaceV3)
                        until not G.AutoRaceV3
                        or not Mob.Parent
                        or Mob.Humanoid.Health <= 0
                ...
                elseif Race == "Skypiea" then
                    ...
                    for _,v in pairs(game.Players:GetPlayers()) do
                        if v ~= plr 
                        and v:FindFirstChild("Data")
                        and v.Data.Race.Value == "Skypiea"
                        and v.Character
                        and v.Character:FindFirstChild("HumanoidRootPart") then

                            repeat
                                task.wait()
                                _tp(v.Character.HumanoidRootPart.CFrame * CFrame.new(0,8,0))
                            until not G.AutoRaceV3
                            or v.Character.Humanoid.Health <= 0
                        end
                    end
...
Race:AddButton({
    Name = "Load & Teleport Temple",
    Description = "Chờ Temple load rồi TP",
    Callback = function()
        local player = game.Players.LocalPlayer
        local char = player.Character or player.CharacterAdded:Wait()
        local hrp = char:WaitForChild("HumanoidRootPart")

        -- TP đến gần Temple để game stream map
        hrp.CFrame = CFrame.new(28286, 14895, 600)

        task.spawn(function()
            local temple

            for i = 1, 100 do
                temple = workspace:FindFirstChild("Map")
                    and workspace.Map:FindFirstChild("Temple of Time", true)
...
AimPlayers = GetSetting("AimPlayers_Save", false)
AimMobs = GetSetting("AimMobs_Save", false)
...
Player:AddToggle({
    ...
    Callback = function(v)
        ...
        if v then
            AimMobs = false
            _G.SaveData["AimMobs_Save"] = false
            SaveSettings()
            v1:SetNPCSilentAim(false)
        end
...
Player:AddToggle({
    ...
    Callback = function(v)
        ...
        if v then
            AimPlayers = false
            _G.SaveData["AimPlayers_Save"] = false
            SaveSettings()
            v1:SetPlayerSilentAim(false)
        end
